...
.PARAMETER SecretStorageMethod
The method used to store the Falcon API credentials. Options are 'SecretsManager' or 'ParameterStore' [default: 'SecretsManager']
.PARAMETER SecretsManagerSecretName
The name of the Secrets Manager secret containing the Falcon API credentials [default: $null]
.PARAMETER FalconCloud
CrowdStrike Falcon OAuth2 API Hostname [default: autodiscover]
.PARAMETER SSMFalconCloud
CrowdStrike Falcon cloud location SSM parameter key
.PARAMETER FalconClientId
CrowdStrike Falcon OAuth2 API Client Id [Required if FalconAccessToken is not provided]
.PARAMETER SSMFalconClientId
CrowdStrike Falcon cloud client ID SSM parameter key
.PARAMETER FalconClientSecret
CrowdStrike Falcon OAuth2 API Client Secret [Required if FalconAccessToken is not provided]
.PARAMETER SSMFalconClientSecret
CrowdStrike Falcon cloud secret SSM parameter key
.PARAMETER FalconCid
...
param(
    ...
    [string] $SecretStorageMethod = 'SecretsManager',
...
begin {
    ...
    function Sanitize-InputParams {
        ...
        foreach ($param in $inputParams) {
            ...
            $paramValue = $paramValue.Trim()  # Trim spaces (equivalent to `xargs` in Bash)
            Set-Variable -Name $param -Value $paramValue
        }
    }
...
# ------------------------------------------------------------------------------------------------------------------------
    function Get-SSMParameter {
        ...
        try {
            # Call AWS CLI to retrieve the parameter
    ...
    function Get-Secret {
        ...
        try {
            # Call AWS CLI to retrieve the secret value
    ...
    function Get-ValueFromSecret {
        ...
        try {
            # Convert the secret string to a PowerShell object
            $secretObject = $Secret | ConvertFrom-Json
            
            # Try to retrieve the value by key
            if ($secretObject.PSObject.Properties[$Key]) {
...
process {
    ...
    switch ($SecretStorageMethod) {
        ...
        "ParameterStore" {
            # Retrieve the parameters from Parameter Store
        ...
        default {
            Write-Error "Invalid SecretStorageMethod: $SecretStorageMethod"
            exit 1
        }
    }
    ...
    $proxy = ""
    if (![string]::IsNullOrEmpty($proxy) -and $proxy.Trim() -ne "") {
    ...
    if ($credsProvided -or $FalconAccessToken) {
        ...
        Write-Output $FalconClientId
        Write-Output $FalconClientSecret
    ...
    if (![string]::IsNullOrEmpty($ProvToken) -and $ProvToken.Trim() -ne "") {
        $InstallParams += " ProvToken=$ProvToken"
    }

    
    if (![string]::IsNullOrEmpty($Tags) -and $Tags.Trim() -ne "") {
        $InstallParams += " GROUPING_TAGS=$Tags"
    }

    if (![string]::IsNullOrEmpty($ProxyHost) -and $ProxyHost.Trim() -ne "") {
        $InstallParams += " APP_PROXYNAME=$ProxyHost"
    }

    if (![string]::IsNullOrEmpty($ProxyPort) -and $ProxyPort.Trim() -ne "") {
